(function($){
	...
	// gallery 페이지 만들기
	// 1. html 문서에서 큰 이미지의 경로/파일을 직접 입력해서 가져오게 하기
	...
	part01LiLink.on('click',function(e){
		...
		//console.log(thisImg)
		bigRec.css({backgroundImage:'url('+thisImg+')'});
  ...
  for(var i=0; i<thumList.length; i+=1){
		thumImg.append('<li><a href=""><span></span></a></li>');
		thumImg.find('li').eq(i).children('a')
		        .css({backgroundImage: 'url('+thumUrl+thumList[i]+')'});
	}
	...
	// ---------------------------------------------------
	// allery를 응용하여, popup창(모달 윈도우) 만들기
	...
	for(i=0; i<galleryList.thumList.length; i++){
		...
		part03Ul.find('li').eq(i).children('a').css({
			backgroundImage:'url('+thumUrl+thumList[i]+')'
		});
	}
